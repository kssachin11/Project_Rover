...
from Layers.L1_App.navigation.polar_to_cartesian import polar_to_cartesian
...
class PointCloudCartesian():
    ...
    def __init__(self, point_cloud_polar: list[Polar], blur_radius: int = 5) -> None:
        ...
        self._data, self._position, self._direction = polar_to_cartesian(Polar.from_array(point_cloud_polar), alignment_iterations=20)
    ...
    def determine_map_position(self, map:Map, scan_radius: int = 6, stages: int = 4, threshold: float = 0.7) -> list[int]:
        ...
        def calcConvolution(areas: list, point_cloud_s: np.ndarray, map_s: np.ndarray) -> list[list, np.ndarray]:
