...
class connect_pksi_INS():
    ...
    def __init__(self) -> None:
        ...
        """
        self.mag_x_, self.mag_y_, self.mag_z_ = 0.0, 0.0, 0.0
        self.acc_x_, self.acc_y_, self.acc_z_ = 0.0, 0.0, 0.0
        self.dgps_type = "rover"
        self.heading_deg = 0.0
        self.config_path = f'{os.path.abspath(os.path.join(os.path.dirname(__file__),"../../../.."))}/Configure.json'
        self.dgps = dgps(self.config_path)

        # Uncomment this code to get the IP address and port from the config file in the workspace
        """ try:
    ...
    def get_mag_data(self):
        self.mag_x_, self.mag_y_, self.mag_z_ = self.dgps.get_data(type = self.dgps_type, 
                                                                   msg_list=[SBP_MSG_MAG_RAW])
        #print(f"Mag X: {self.mag_x_}, Mag Y: {self.mag_y_}, Mag Z: {self.mag_z_}")
        return self.mag_x_, self.mag_y_, self.mag_z_
    
    def get_acc_data(self):
        self.acc_x_, self.acc_y_, self.acc_z_ = self.dgps.get_data(type = self.dgps_type, 
                                                                   msg_list=[SBP_MSG_IMU_RAW])
    ...
    # Uncomment this function to get the magnetometer data from piksi directly
    """ def get_mag_data(self):
    ...
    # Uncomment this function to get the accelerometer data from piksi directly
    """ def get_acc_data(self):
        # Open a connection to Piksi using TCP
        with TCPDriver(self.IP_add, self.port) as driver:
            with Handler(Framer(driver.read, None, verbose=True)) as source:
                ...
                try:
                    ...
                    if msg is not None:
                        print(f'Data Recieving from piksi at IP {msg.sender}')
                        self.acc_x_= msg.acc_x * 8 /32768
                        self.acc_y_= msg.acc_y * 8 /32768
                        self.acc_z_= msg.acc_z * 8 /32768
