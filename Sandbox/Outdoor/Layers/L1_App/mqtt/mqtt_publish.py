...
class MQTT_Publish(metaclass = Singleton_meta):
    ...
    def __init__(self) -> None:
        ...
        self.status = "Not Connected to MQTT Broaker\n"
        try:
    ...
    def connect_mqtt(self):
        ...
        def on_connect(client, userdata, flag, rc):
            if rc == 0:
                ...
                self.status = "Connected to MQTT Broaker: {}\n".format(self.broker)

            else:
                print("Failed to connect, return code %d\n", rc)
                self.status = "Failed to connect, return code {}\n".format(rc)
    ...
    def connect_publisher(self, topic, msg):
        ...
        try:
            client = self.connect_mqtt()  # Remove the trailing comma after 'return client'
            client.loop_start()
            time.sleep(0.1)
